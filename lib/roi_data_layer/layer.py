...
#      print("________________________________________________________________________________________________")
  ...
  def _get_next_minibatch_inds(self):
    ...
    得到下一个batch图片的index，cur是当前的index（已经训练）
    """
    
    if self._cur + cfg.TRAIN.IMS_PER_BATCH >= len(self._roidb):
      self._shuffle_roidb_inds()#超过了roidb的长度，重新打乱
  ...
  def _get_next_minibatch(self):
    ...
    """
    db_inds = self._get_next_minibatch_inds() #得到index
    minibatch_db = [self._roidb[i] for i in db_inds]#将所有roidb[index]组成minibatch_db，已找到图片，
    return get_minibatch(minibatch_db, self._num_classes)#组成blob,  转到minibatch。py
